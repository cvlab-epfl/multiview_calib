...
def draw_epilines(img1_undistorted, img2_undistorted, 
    ...
    for pt1, pt2, l1, l2 in zip(pts1_, pts2_, lines1, lines2): 
        color1 = tuple(int(x) for x in np.random.randint(0, 255, 3))
        color2 = tuple(int(x) for x in np.random.randint(0, 255, 3))
        
        img1_ = _draw_line(img1_, l1, color1, linewidth)
        img2_ = _draw_line(img2_, l2, color1, linewidth)
        
        img1_ = cv2.circle(img1_, tuple(int(x) for x in pt1), radius=markersize, color=color1, thickness=-1)
        img1_ = cv2.circle(img1_, tuple(int(x) for x in pt1), radius=markersize//2, color=(0,0,0), thickness=-1)
        
        img2_ = cv2.circle(img2_, tuple(int(x) for x in pt2), radius=markersize, color=color1, thickness=-1)
